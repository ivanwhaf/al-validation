...
def al_qbc():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model1 = torch.load('cifar10_1.pth').to(device)
    model2 = torch.load('cifar10_2.pth').to(device)
    model3 = torch.load('cifar10_3.pth').to(device)
    ...
    for trail in range(trails):
        ...
        torch.save(model, os.path.join(
            output_path, "cifar10_qbc_trail"+str(trail)+".pth"))
        # ============================retrain===============================
